...
def in_flight_local_model(state: RunState) -> int:
    """Count workers currently running on the on-device model."""
    return sum(
        1 for t in state.tasks.values()
        if t.status == "in_flight" and t.target == "local-model"
    )


def local_model_slots(state: RunState, config: LegionConfig) -> int:
    """Extra overflow slots available for local-model workers right now.

    This is ON TOP of the session/cloud cap (current_max_workers) — local-model
    workers run on-device and don't share the Anthropic rate limit, so they're
    counted separately rather than against the hard cap.
    """
    if not config.local_model.enabled:
        return 0
    return max(0, config.local_model.max_workers - in_flight_local_model(state))
...
def local_model_available(config: LegionConfig) -> bool:
    ...
    spawning workers that would immediately fail.
    """
    if not config.local_model.enabled:
        return False
    import urllib.request

    url = config.local_model.base_url.rstrip("/") + "/health"
    try:

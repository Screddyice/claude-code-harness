...
def is_simple(task: Task, dispatch_cfg: DispatchConfig) -> bool:
    """True if a task is small/safe enough to hand to the local (on-device) model.

    Mirrors the "stay local" criteria in route(): excludes browser/throughput
    and always-cloud work, and requires the task be trivial or under the
    file + minute thresholds. Used to filter overflow tasks when
    local_model.offload == "simple".
    """
    title_and_spec = f"{task.title}\n{task.spec}"

    if BROWSER_RE.search(title_and_spec) or THROUGHPUT_RE.search(title_and_spec):
        return False
    ...
    # Small enough -> simple
    return (
        len(task.files_touched) < dispatch_cfg.local_file_threshold
        and task.estimated_minutes < dispatch_cfg.cloud_minutes_threshold
    )

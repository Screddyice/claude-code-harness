...
def spawn_local(task: Task, base_branch: str, branch_prefix: str, auth_mode: str = "session", repo_url: str = "", retros=None, model: str | None = None, base_url: str | None = None) -> dict:
    ...
    # local-model worker: route this `claude -p` to the on-device model.
    if model:
        cmd += ["--model", model]
...
def poll_local(task: Task, base_branch: str) -> dict | None:
    ...
    # Open PR (against the actual base branch). Label local-model workers
    # distinctly so the PR shows which fleet shipped it.
    pr_body = _make_pr_body(task, target=task.target or "local")
    pr_result = _run([
        "gh", "pr", "create",
        "--base", base_branch,
        "--head", branch,
...
def spawn(task: Task, target: str, repo_url: str, base_branch: str, branch_prefix: str, auth_mode: str = "session", model: str | None = None, base_url: str | None = None) -> dict:
    ...
    if target == "local-model":
        # Overflow worker on the on-device model. Reuses the local subprocess
        # machinery; `model` is what flips it to local-model.
        return spawn_local(task, base_branch, branch_prefix, auth_mode=auth_mode, repo_url=repo_url, retros=retros, model=model, base_url=base_url)
    if target == "cloud":
...
def poll(task: Task, base_branch: str) -> dict | None:
    # local-model workers are local subprocesses — same poll path as `local`.
    if task.target in ("local", "local-model"):

...
The reviewer does not unload its model when a review finishes, so the model
stays resident until Ollama evicts it. `qwen3.5:4b-64k` holds about 3 GB at
`OLLAMA_NUM_PARALLEL=2`, and more as you raise the slot count, since Ollama
charges KV cache as `num_ctx x OLLAMA_NUM_PARALLEL`. That resident copy competes
with anything else you load, including an `llmjury` council. On a host where
those two overlap, set `LOCAL_REVIEW=0` for the session, pick a smaller
`LOCAL_REVIEW_MODEL`, or run `ollama stop <model>` before a council solve.

...
check deny  'sudo -i rm -rf .claude-harness'                 'a recursive delete via sudo with an unlisted option'
check deny  'sudo -A rm -rf .claude-harness'                 'a recursive delete via sudo with an askpass option'
check deny  'sudo -EH rm -rf .claude-harness'                'a recursive delete via sudo with combined options'
check deny  'xargs -0 rm -rf .claude-harness'                'a recursive delete via xargs with options'
check deny  'xargs -L1 rm -rf .claude-harness'               'a recursive delete via xargs with an unlisted option'
check deny  'xargs -J repl rm -rf .claude-harness'           'a recursive delete via xargs with a replacement option'
